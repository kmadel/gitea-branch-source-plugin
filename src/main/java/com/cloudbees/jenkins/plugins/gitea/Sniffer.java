...
/**
 * Pattern matching on Gitea organization folder member {@link Item}s.
 *
 * @author Kurt Madel
 */
class Sniffer {
    static class OrgMatch {
        ...
        final GiteaSCMNavigator scm;

        public OrgMatch(OrganizationFolder folder, GiteaSCMNavigator scm) {
            this.folder = folder;
            this.scm = scm;
        }
    }
    ...
    public static OrgMatch matchOrg(Object item) {
        if (item instanceof OrganizationFolder) {
            ...
            if (/* could be called from constructor */navigators != null && navigators.size() > 0) {
                ...
                if (n instanceof GiteaSCMNavigator) {
                    return new OrgMatch(of, (GiteaSCMNavigator) n);
                }
            }
        }
        return null;
